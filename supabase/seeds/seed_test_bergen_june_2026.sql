-- ============================================================================
-- Seed data — "Bergen, June 2026" trip for the test users
-- (test@test.com and test2@test.com)
--
-- A made-up week in Bergen: a handful of restaurant meals, beers and cocktails,
-- some souvenir/clothing purchases, and a few activities. Both users get the
-- same trip on the same dates, but with different ratings and beverages.
-- Everything is scoped to a single project per user, so it's easy to delete
-- again (see bottom).
--
...
begin
  ...
  -- Cocktails
  insert into memoir_drink_items (user_id, name, drink_type, default_rating, notes)
  values (v_user_id, 'Piña Colada', 'cocktail', 8, 'Holiday in a glass')
  on conflict (user_id, lower(name)) do update set notes = excluded.notes
  returning id into d_pina;

  insert into memoir_drink_items (user_id, name, drink_type, default_rating, notes)
  values (v_user_id, 'Strawberry Daiquiri', 'cocktail', 8, NULL)
  on conflict (user_id, lower(name)) do update set drink_type = excluded.drink_type
  returning id into d_daiquiri;
  ...
  values
    ...
    -- Cocktails (count_* sizes are beer-specific, so they stay 0; quantity is the glass count)
    (v_user_id, v_project_id, '2026-06-10', d_pina,     'cocktail', 12.0, 0, 0, 0, 0, 0, 1, 8, 165.00, 'Bergen', 'Norway', 'Sunset cocktail by the harbour'),
    (v_user_id, v_project_id, '2026-06-12', d_daiquiri, 'cocktail', 14.0, 0, 0, 0, 0, 0, 2, 9, 330.00, 'Bergen', 'Norway', 'Pre-dinner at the tasting menu');
...
-- ============================================================================
-- Same trip, second test user (test2@test.com): identical dates and structure,
-- but a different palate — different ratings and a different beverage line-up.
-- ============================================================================
...
begin
  -- --------------------------------------------------------------------------
  -- Resolve the second test user. Abort cleanly if the account doesn't exist.
  -- --------------------------------------------------------------------------
  select id into v_user_id from auth.users where email = 'test2@test.com';
  if v_user_id is null then
    raise exception 'No auth.users row for test2@test.com — create the account first.';
  end if;
  ...
  -- --------------------------------------------------------------------------
  -- Reusable items (different default ratings than the first user)
  -- --------------------------------------------------------------------------

  -- Restaurants
  insert into memoir_venues (user_id, name, default_rating, latitude, longitude, address, notes)
  values (v_user_id, 'Bryggeloftet & Stuene', 7, 60.3972, 5.3225, 'Bryggen 11, 5003 Bergen', 'Classic Norwegian, on the old wharf')
  on conflict (user_id, lower(name)) do update set notes = excluded.notes
  returning id into r_bryggeloftet;

  insert into memoir_venues (user_id, name, default_rating, latitude, longitude, address, notes)
  values (v_user_id, 'Pingvinen', 8, 60.3897, 5.3215, 'Vaskerelven 14, 5014 Bergen', 'Cosy local pub, traditional dishes')
  on conflict (user_id, lower(name)) do update set notes = excluded.notes
  returning id into r_pingvinen;

  insert into memoir_venues (user_id, name, default_rating, latitude, longitude, address, notes)
  values (v_user_id, 'Bare Vestland', 10, 60.3935, 5.3242, 'Torgallmenningen 2, 5014 Bergen', 'Modern west-coast tasting menu')
  on conflict (user_id, lower(name)) do update set notes = excluded.notes
  returning id into r_bare;

  insert into memoir_venues (user_id, name, default_rating, latitude, longitude, address, notes)
  values (v_user_id, 'Trekroneren', 8, 60.3925, 5.3245, 'Kong Oscars gate 1, 5017 Bergen', 'Famous reindeer hot dog stand')
  on conflict (user_id, lower(name)) do update set notes = excluded.notes
  returning id into r_trekroneren;

  insert into memoir_venues (user_id, name, default_rating, latitude, longitude, address, notes)
  values (v_user_id, 'Fish Me', 9, 60.3955, 5.3251, 'Zachariasbryggen, 5014 Bergen', 'Seafood by the harbour')
  on conflict (user_id, lower(name)) do update set notes = excluded.notes
  returning id into r_fishme;

  -- Drinks — a different line-up: leans on cocktails and a couple of beers
  insert into memoir_drink_items (user_id, name, drink_type, default_rating, notes)
  values (v_user_id, 'Hansa Pils', 'beer', 6, 'The local Bergen lager')
  on conflict (user_id, lower(name)) do update set notes = excluded.notes
  returning id into d_hansa;

  insert into memoir_drink_items (user_id, name, drink_type, default_rating, notes)
  values (v_user_id, 'Voss Pils', 'beer', 7, NULL)
  on conflict (user_id, lower(name)) do update set drink_type = excluded.drink_type
  returning id into d_voss;

  insert into memoir_drink_items (user_id, name, drink_type, default_rating, notes)
  values (v_user_id, '7 Fjell IPA', 'beer', 8, 'Local Bergen microbrewery')
  on conflict (user_id, lower(name)) do update set notes = excluded.notes
  returning id into d_7fjell;

  insert into memoir_drink_items (user_id, name, drink_type, default_rating, notes)
  values (v_user_id, 'Kinn Vestkyst', 'beer', 7, 'Pale ale from Sunnfjord')
  on conflict (user_id, lower(name)) do update set notes = excluded.notes
  returning id into d_kinn;

  -- Cocktails
  insert into memoir_drink_items (user_id, name, drink_type, default_rating, notes)
  values (v_user_id, 'Piña Colada', 'cocktail', 10, 'Holiday in a glass')
  on conflict (user_id, lower(name)) do update set notes = excluded.notes
  returning id into d_pina;

  insert into memoir_drink_items (user_id, name, drink_type, default_rating, notes)
  values (v_user_id, 'Strawberry Daiquiri', 'cocktail', 9, NULL)
  on conflict (user_id, lower(name)) do update set drink_type = excluded.drink_type
  returning id into d_daiquiri;

  -- Food items
  insert into memoir_food_items (user_id, name, default_rating, notes)
  values (v_user_id, 'Bergen fish soup', 8, 'Creamy local classic')
  on conflict (user_id, lower(name)) do update set notes = excluded.notes
  returning id into f_fishsoup;

  insert into memoir_food_items (user_id, name, default_rating, notes)
  values (v_user_id, 'Reindeer stew', 9, NULL)
  on conflict (user_id, lower(name)) do update set default_rating = excluded.default_rating
  returning id into f_reindeer;

  insert into memoir_food_items (user_id, name, default_rating, notes)
  values (v_user_id, 'Fish cakes', 6, NULL)
  on conflict (user_id, lower(name)) do update set default_rating = excluded.default_rating
  returning id into f_fishcake;

  insert into memoir_food_items (user_id, name, default_rating, notes)
  values (v_user_id, 'Reindeer hot dog', 8, 'From Trekroneren')
  on conflict (user_id, lower(name)) do update set notes = excluded.notes
  returning id into f_hotdog;

  insert into memoir_food_items (user_id, name, default_rating, notes)
  values (v_user_id, 'Grilled catch of the day', 10, NULL)
  on conflict (user_id, lower(name)) do update set default_rating = excluded.default_rating
  returning id into f_wholefish;

  -- Activities
  insert into memoir_activity_items (user_id, name, default_rating, notes)
  values (v_user_id, 'Fløibanen funicular', 8, 'Up Mount Fløyen for the view')
  on conflict (user_id, lower(name)) do update set notes = excluded.notes
  returning id into a_floibanen;

  insert into memoir_activity_items (user_id, name, default_rating, notes)
  values (v_user_id, 'Ulriken cable car', 9, NULL)
  on conflict (user_id, lower(name)) do update set default_rating = excluded.default_rating
  returning id into a_ulriken;

  insert into memoir_activity_items (user_id, name, default_rating, notes)
  values (v_user_id, 'Bryggen walking tour', 7, 'Hanseatic wharf, UNESCO site')
  on conflict (user_id, lower(name)) do update set notes = excluded.notes
  returning id into a_bryggen;

  insert into memoir_activity_items (user_id, name, default_rating, notes)
  values (v_user_id, 'Mostraumen fjord cruise', 9, NULL)
  on conflict (user_id, lower(name)) do update set default_rating = excluded.default_rating
  returning id into a_fjord;

  insert into memoir_activity_items (user_id, name, default_rating, notes)
  values (v_user_id, 'Bergen Aquarium', 7, NULL)
  on conflict (user_id, lower(name)) do update set default_rating = excluded.default_rating
  returning id into a_aquarium;

  -- --------------------------------------------------------------------------
  -- Food entries (same dates, different ratings)
  -- --------------------------------------------------------------------------
  insert into memoir_food_entries
    (user_id, project_id, entry_date, meal_type, source, food_item_id, venue_id, starter, main_course, dessert, rating, cost, notes)
  values
    (v_user_id, v_project_id, '2026-06-08', 'dinner', 'venue', f_fishsoup,  r_bryggeloftet, 'Bergen fish soup', 'Bacalao', 'Cloudberry cream', 7, 645.00, 'Solid but a touch salty'),
    (v_user_id, v_project_id, '2026-06-09', 'lunch',  'venue', f_hotdog,    r_trekroneren,  NULL, 'Reindeer hot dog', NULL, 8, 95.00,  'Better than expected'),
    (v_user_id, v_project_id, '2026-06-09', 'dinner', 'venue', f_reindeer,  r_pingvinen,    NULL, 'Reindeer stew', 'Apple cake', 9, 410.00, 'Favourite meal so far'),
    (v_user_id, v_project_id, '2026-06-10', 'lunch',  'venue', f_fishcake,  r_pingvinen,    NULL, 'Fish cakes with potatoes', NULL, 6, 245.00, 'A bit dry'),
    (v_user_id, v_project_id, '2026-06-11', 'dinner', 'venue', f_wholefish, r_fishme,       'Shrimp', 'Grilled halibut', NULL, 10, 720.00, 'Perfectly cooked'),
    (v_user_id, v_project_id, '2026-06-12', 'dinner', 'venue', NULL,        r_bare,         'Scallop', 'West-coast tasting menu', 'Brown cheese ice cream', 10, 1290.00, 'Worth every krone'),
    (v_user_id, v_project_id, '2026-06-13', 'breakfast', 'venue', NULL,     NULL,           NULL, 'Skillingsbolle & coffee', NULL, 7, 110.00, 'Cinnamon bun by the fish market');

  -- --------------------------------------------------------------------------
  -- Drink entries (same dates, different beverages and ratings)
  -- --------------------------------------------------------------------------
  insert into memoir_drink_entries
    (user_id, project_id, entry_date, drink_item_id, drink_type, abv,
     count_033l, count_04l, count_05l, count_0568l, count_06l,
     quantity, rating, cost, city, country, notes)
  values
    (v_user_id, v_project_id, '2026-06-08', d_voss,     'beer',     4.5, 0, 0, 1, 0, 0, 1, 7, 98.00,  'Bergen', 'Norway', 'With dinner'),
    (v_user_id, v_project_id, '2026-06-09', d_kinn,     'beer',     4.7, 1, 0, 0, 0, 0, 1, 7, 89.00,  'Bergen', 'Norway', NULL),
    (v_user_id, v_project_id, '2026-06-10', d_pina,     'cocktail', 12.0, 0, 0, 0, 0, 0, 2, 10, 330.00, 'Bergen', 'Norway', 'Two by the harbour at sunset'),
    (v_user_id, v_project_id, '2026-06-11', d_daiquiri, 'cocktail', 14.0, 0, 0, 0, 0, 0, 1, 9, 165.00, 'Bergen', 'Norway', 'With the seafood'),
    (v_user_id, v_project_id, '2026-06-12', d_pina,     'cocktail', 12.0, 0, 0, 0, 0, 0, 1, 9, 165.00, 'Bergen', 'Norway', 'Splurge night'),
    (v_user_id, v_project_id, '2026-06-13', d_hansa,    'beer',     4.5, 0, 0, 1, 0, 0, 1, 6, 95.00,  'Bergen', 'Norway', 'Nightcap');

  -- --------------------------------------------------------------------------
  -- Activity entries (same dates, different ratings)
  -- --------------------------------------------------------------------------
  insert into memoir_activity_entries
    (user_id, project_id, entry_date, activity_item_id, rating, cost, notes)
  values
    (v_user_id, v_project_id, '2026-06-08', a_bryggen,   7, 0.00, 'Afternoon stroll through the old Hanseatic wharf. Free to wander'),
    (v_user_id, v_project_id, '2026-06-09', a_floibanen, 8, 160.00, 'Funicular up Mount Fløyen, hiked back down'),
    (v_user_id, v_project_id, '2026-06-10', a_fjord,     9, 690.00, 'Mostraumen fjord cruise from the harbour. Waterfalls and tidal rapids'),
    (v_user_id, v_project_id, '2026-06-11', a_ulriken,   9, 295.00, 'Cable car up Mount Ulriken — best view of the trip'),
    (v_user_id, v_project_id, '2026-06-12', a_aquarium,  7, 320.00, 'Bergen Aquarium in the morning');

  -- --------------------------------------------------------------------------
  -- Purchases (same dates, slightly different picks)
  -- --------------------------------------------------------------------------
  insert into memoir_purchase_entries
    (user_id, project_id, entry_date, item_name, category, cost, notes)
  values
    (v_user_id, v_project_id, '2026-06-08', 'Umbrella',               'other',       299.00,  'Rained the first day'),
    (v_user_id, v_project_id, '2026-06-10', 'Norwegian wool sweater', 'clothes',     2190.00, 'Lusekofte from Husfliden'),
    (v_user_id, v_project_id, '2026-06-10', 'Fridge magnet',          'souvenir',    79.00,   'For the collection'),
    (v_user_id, v_project_id, '2026-06-11', 'Bryggen postcards',      'souvenir',    89.00,   'Pack of 6'),
    (v_user_id, v_project_id, '2026-06-12', 'Jar of cloudberry jam',  'other',       159.00,  'From the fish market'),
    (v_user_id, v_project_id, '2026-06-13', 'Phone charger',          'electronics', 249.00,  NULL);

  raise notice 'Seeded Bergen 2026 trip for % (project %)', v_user_id, v_project_id;
end $$;

-- ============================================================================
-- To remove this seed data later (covers both test users):
--   delete from memoir_projects
--    where name = 'Bergen 2026'
--      and user_id in (select id from auth.users
--                       where email in ('test@test.com', 'test2@test.com'));
